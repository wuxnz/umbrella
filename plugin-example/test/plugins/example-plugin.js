...
class ExamplePlugin {
    constructor() {
        this.baseUrl = 'https://ww24.gogoanimes.fi';
    ...
    search(query, page) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const url = `${this.baseUrl}/search.html?keyword=${query}&page=${page || 1}`;
    ...
    getItemDetails(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ohterNamesRegex = /<p class="type other-name">[\s\S]*<span>Other name:[\s\S]*?<\/span>[\s\S]*?<a.* title="(.*?)<\/a>[\s\S]*?<\/p>/;
            ...
            if (!episodesResponse) {
                ...
                const episodesRegex = /<a href="([\s\S* ]*?)"[\s\S]*?class.*>[\s\S]*?<div class="name"><span>(.*?)<\/span>([\s\S]*?)<\/div>[\s\S]*?<div class="cate">(.*)?<\/div>/g;
    ...
    getItemMedia(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ul = [...episodePageResponse.matchAll(allUlRegex)].filter(item => item[0].includes('data-video'));
            if (ul.length !== 1) {
                return [];
            }
            const ulText = ul[0][0];
            const embedInfoRegex = /<li class=".*">[\s\S]*?<a.*data-video="(.*?)".*>[\s\S]*?<i class="iconlayer-(.*?)">?<\/i>(.*)<span>/g;
            const embedInfoList = [...ulText.matchAll(embedInfoRegex)].map(item => ({
                url: new URL(item[1]),
                iconUrl: `${this.baseUrl}img/${item[2]}.png`,
                name: item[3],
            }));
            const vidHideSources = embedInfoList.filter(item => item['name'].toLowerCase().includes('vidhide'));
            var itemMedia = [];
            for (const source of vidHideSources) {
                const videoUrl = yield this.vidHideExtractor(source['url']);
                itemMedia.push({
                    type: 'RawVideo',
                    url: videoUrl,
                    name: source['name'],
...
module.exports = {
    ...
    getItemMedia: (id) => __awaiter(this, void 0, void 0, function* () { return new ExamplePlugin().getItemMedia(id); }),
};
